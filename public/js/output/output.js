	...
	buildHistory:function buildHistory(){
		var runs = $.get("/database/all-runs").done(function(data){
			...
			for(var i = 0;i < run_list.length;i++){
				...
				//link
				var link = $("<li><div class='history-box'><a href=" + d.local_url + " target='_blank'>LINK</div></li>");
				$(hist_ul).append(link);
	...
	var runs = $.get("/database/all-runs").done(function(data){
		...
		if (filter[0] != null || statusFilter[0] != null){
			for(i = 0; i <= filter.length; i++){
				...
				filterModel = filter[i] + ".R";
				for(j = 0; j <= statusFilter.length; j++){
					filterStatus = statusFilter[j];
					for(n = 0; n < run_list.length; n++){	
						 if (run_list[n].status == filterStatus || statusFilter[0] == null){
							if (run_list[n].step == filterValue || run_list[n].script == filterModel || filter[0] == null){
								var d = data.runs[n];
								var hist_item = $("<li class='history-item'></li>");
								var hist_ul = $("<ul></ul>");				
								//alias
