...
function runModel(stepID, step, stepDir) {
  ...
  var script = config.server.ModelsDir + '/r/' + step.script;
	...
	for(var i = 0;i < seq.length;i++){
		if(seq[i].exists){ }
			...
			for(var k = 0; k < seq.length; k++){
				var step_dir = seq[k].step + '_dir';
				seq[i].preceding[step_dir] = seq[k].url;
				seq[i].settings.preceding[step_dir] = seq[k].url;
